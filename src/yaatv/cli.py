...
def quality_warnings(
    ...
    # Image scale checks only apply when a cover is present (not color-only).
...
def run(
    ...
    with ExitStack() as stack:
        ...
        if not args.no_warn:
            ...
            # Always run quality_warnings so low-bitrate audio is reported even
            # for color-only output (image_size is None).
